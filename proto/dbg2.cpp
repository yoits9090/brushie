...
struct Dec {
  ...
  uint32_t bit(uint16_t f1){
    ...
    uint32_t b = slot >= (4096u-f1)*16u ? 1u : 0u;
    uint32_t f = (b ? f1 : (4096u-f1))*16u;
    uint32_t c = b ? (4096u-f1)*16u : 0u;
    x = f * (x >> 16) + slot - c;
...
int main(){
  ...
  std::vector<uint16_t> fused(bits.size());
  uint16_t p = 2048;
  for (size_t i = 0; i < bits.size(); ++i) { fused[i] = p; p = adapt(p, bits[i], 5); }
  std::vector<uint8_t> out;
  Enc enc(out);
  for (size_t i = bits.size(); i-- > 0;) enc.bit(fused[i], bits[i]);
  enc.flush();
  printf("bytes=%zu chunks=%zu\n", out.size(), out.size()/2 - 2);
  Dec dec(out.data(), out.size());
  uint16_t p2 = 2048;
  for (size_t i = 0; i < bits.size(); ++i) {
    uint32_t d = dec.bit(p2);
    if (d != bits[i]) printf("MISMATCH %zu want %u got %u\n", i, bits[i], d);
    p2 = adapt(p2, bits[i], 5);
  }
  printf("done\n");
  return 0;
}
