...
def _walk(dec, probs, audit, dest, stride, tw, th, version, mode,
          ...
          step_c=1, step_p=1):
    if mode == 24 and version >= 7:
        k0 = k0 & 15  # top 2 bits carry the rANS initial-state size flag
    k = k0
    ...
    if mode in (12, 24):
        ...
        return
    if mode == 16:
        # Flat-block base mode (v6): 4x4 blocks; a flat block codes one value
        # (residual from left/above block value), otherwise per-pixel coding.
        kB = 4
        bw, bh = (tw + kB - 1) // kB, (th + kB - 1) // kB
        flat = [0] * (bw * bh)
        bval = [0] * (bw * bh)
        ...
        for by in range(bh):
            for bx in range(bw):
                ...
                flat[by * bw + bx] = dec.bit(probs, fctx, "blk")
        v_mag_sum, v_mag_count, vk = 0, 0, 0
        for by in range(bh):
            for bx in range(bw):
                idx = by * bw + bx
                y0, x0 = by * kB, bx * kB
                ...
                if flat[idx]:
                    ...
                    rv = 0
                    if dec.bit(probs, vctx, "sig"):
                        sctx = 57 + (1 if lr < 0 else 0) + (2 if ar < 0 else 0)
                        sign = dec.bit(probs, sctx, "sign")
                        ...
                        qq = 0
                        while True:
                            # mode 16 predates the v6 unary-rate update: 1/32
                            if dec.bit(probs, 20 + min(qq, 23), "unary"): break
                            qq += 1
                        m = qq << kk
                        for i in range(kk):
                            m |= dec.bit(probs, 44 + i, "rem") << i
                        rv = m + 1
                        if sign: rv = -rv
                        v_mag_sum += m
                        v_mag_count += 1
                ...
                for yy in range(y0, y1):
                    for xx in range(x0, x1):
                        raw_ctx = sig_context(dest, stride, xx, yy)
                        s = dec.bit(probs, 8 + raw_ctx, "sig")
                        q = 0
                        if s:
...
def audit_stream(path: Path):
    ...
    for ci in range(chunk_count):
        ...
        if band == 4:
            ...
            for bi in range(3):
                if modes[bi] == 0:
                    # absent section: band left zero at decode (mode byte 0)
                    cursor += secs[bi]
                    continue
